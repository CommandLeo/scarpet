...
create(name, item_string) -> (
    ...
    if(length(invalid_items) > 0, exit(print(player(), str('§cInvalid items: %s', join(', ', invalid_items)))));
    createTable(name, items);
);

createFromContainer(name, position) -> (
    b = position || query(player(), 'trace', 5, 'blocks');
    if(!b || inventory_has_items(b) == null, exit(print(player(), '§cThat block is not a container!')));
    if(!inventory_has_items(b), exit(print(player(), '§cCant\'t create a table from an empty container')));
    items = {};
...
createFromArea(name, start, end) -> (
    ...
    if(!items, exit(print(player(), '§cThe selected area is empty')));
    if(ignored_blocks, print(player(), str('§8» §7The following blocks were ignored: %s', join(' ', keys(ignored_blocks)))));
    createTable(name, keys(items));
);

createTable(name, items) -> (
    if(list_files('', 'text')~name != null, exit(print(player(), '§cThat name is already in use!')));
    write_file(name, 'text', items);
    print(player(), '§aTable was successfully created');
    run(str('playsound minecraft:entity.evoker.cast_spell master %s', player()));
);

deleteTable(table) -> (
    if(list_files('', 'text')~table == null, exit(print(player(), '§cThat table doesn\'t exist')));
    delete_file(table, 'text');
    print(player(), str('§aTable %s was deleted', table));
    run(str('minecraft:item.shield.break master %s', player()));
);

mergeTables(table1, table2, name) -> (
    if(list_files('', 'text')~table1 == null, exit(print(player(), str('§c%s doesn\'t exist', table1))));
    if(list_files('', 'text')~table2 == null, exit(print(player(), str('§c%s doesn\'t exist', table2))));
    if(table1 == table2, exit(print(player(), '§cThe two tables can\'t be the same')));
    if(list_files('', 'text')~name != null, exit(print(player(), '§cThat name is already in use!')));
...
listTables() -> print(player(), format(reduce(list_files('', 'text'), [..._a, if(_i == 0, '', 'g , '), str('#9B59B6 %s', _), str('^g /%s info %s', system_info('app_name'), _), str('?/%s info %s', system_info('app_name'), _)], ['f » ', 'g Available tables: '])));

info(table) -> (
    if(list_files('', 'text')~table == null, exit(print(player(), '§cThat table doesn\'t exist')));
    items = readTable(table);
    print(player(), format(reduce(items, [..._a, if(length(items) < 10, ' \n', if(_i == 0, '', 'g , ')), str('g %s%s', if(length(items) < 10, '   ', ''),_), str('!/give @s %s', _)], ['f » ', 'g The table ', str('#9B59B6 %s ', table), 'g contains the following items: '])));
);
...
exportDatapack(mode, table) -> (
    ...
    print(player(), format('f » ', 'g Table ', str('#9B59B6 %s', table), 'g  was exported as ', str('#9B59B6 %s.zip', datapack_name), 'g  with mode ', str('#9B59B6 %s', mode)));
);
